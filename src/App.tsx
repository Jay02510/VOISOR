...
export default function App() {
    ...
    return (
      <div className="space-y-2">
        {lines.map((line, idx) => {
          ...
          if (isBullet) {
            return (
              <div key={idx} className={`flex gap-2 ml-2 items-start ${darkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>
          ...
          if (cleanLine.startsWith('## ')) {
            return <h3 key={idx} className={`text-[14px] font-black ${darkMode ? 'text-white border-zinc-800' : 'text-zinc-900 border-zinc-200'} mt-4 mb-2 tracking-tight font-sans border-b pb-1`}>{cleanLine.substring(3)}</h3>;
          }

          return <p key={idx} className={`${darkMode ? 'text-zinc-300' : 'text-zinc-700'} leading-relaxed text-[12.5px] font-sans`}>{renderedLine}</p>;
        })}
      </div>
    );
  };
  ...
  return (
    <div className={`relative min-h-screen transition-colors duration-300 ${
      darkMode 
        ? 'bg-[#07080a] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-zinc-900 via-zinc-950 to-black text-[#f4f4f5]' 
        : 'bg-[#f4f6f8] text-zinc-800'
    } overflow-hidden selection:bg-blue-500/30 selection:text-white flex flex-col font-sans`}>
      
      {/* Decorative clean background subtle elements */}
      <div className={`absolute top-1/4 left-1/4 w-96 h-96 ${darkMode ? 'bg-blue-500/5' : 'bg-blue-500/[0.03]'} rounded-full filter blur-[120px] pointer-events-none`} />
      <div className={`absolute bottom-1/4 right-1/4 w-96 h-96 ${darkMode ? 'bg-zinc-800/5' : 'bg-zinc-350/[0.04]'} rounded-full filter blur-[120px] pointer-events-none`} />

      {/* Header Bar */}
      <header className={`w-full max-w-7xl mx-auto px-6 h-16 flex items-center justify-between border-b ${
        darkMode ? 'border-zinc-900/60' : 'border-zinc-200'
      } shrink-0 z-10`}>
        <div className="flex items-center gap-2 cursor-pointer select-none" onClick={handleRestart}>
          ...
          <div className="flex items-center gap-2 ml-1">
            <span className={`${darkMode ? 'text-zinc-700' : 'text-zinc-300'} font-light text-base`}>|</span>
            <span className={`${darkMode ? 'text-zinc-300' : 'text-zinc-600'} font-black text-sm tracking-widest uppercase font-mono`}>
              VOISOR
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-all active:scale-[0.95] flex items-center justify-center cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
              darkMode 
          ...
          {step > 1 && (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all active:scale-[0.97] flex items-center gap-1.5 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                darkMode
                  ? 'border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-900/80'
                  : 'border-zinc-200 text-zinc-600 hover:text-zinc-800 hover:bg-zinc-50 shadow-xs'
              }`}
            >
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 py-6 flex flex-col items-center justify-center relative z-10 overflow-hidden">
        ...
        <div className={`w-full h-[78vh] rounded-2xl ${
          darkMode ? 'bg-zinc-900/45 border-zinc-800/80' : 'bg-white border-zinc-200 shadow-xl shadow-zinc-200/40'
        } border backdrop-blur-xl flex flex-col overflow-hidden relative`}>
          
          {/* Elevated Chat Header (Adapts dynamically) */}
          <div className={`px-6 py-4 border-b ${
            darkMode ? 'border-zinc-800/60 bg-zinc-950/40' : 'border-zinc-150 bg-zinc-50'
            ...
            <div className="flex items-center gap-3">
              <div className={`h-9 w-9 rounded-xl flex items-center justify-center border ${
                step === 3
                  ? (currentView === 'CANDIDATE' 
                      ? (darkMode ? 'bg-zinc-800 text-white border-zinc-700' : 'bg-zinc-100 text-zinc-800 border-zinc-200') 
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className={`font-extrabold text-sm ${darkMode ? 'text-white' : 'text-zinc-900'}`}>
                    {step === 3 ? (
                ...
                <p className={`text-[10.5px] ${darkMode ? 'text-zinc-400' : 'text-zinc-500'} font-medium`}>
                  {step === 3 ? (
                    <>대상 지원자: <strong className={darkMode ? 'text-zinc-200' : 'text-zinc-800'}>{selectedReport?.candidateName || 'Sam 지원자'}</strong> &bull; {selectedReport?.targetRole || 'B2B SaaS 테크 영업'}</>
                  ) : (
                    '대화형 설정을 진행하는 중입니다.'
                  )}
                </p>
              ...
              {step === 3 && (
                <>
                  <button
                    ...
                    className={`text-[10.5px] font-extrabold px-2.5 py-1 rounded-lg border transition-all active:scale-[0.97] cursor-pointer motion-safe:animate-fade-in focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                      darkMode 
                        ? 'bg-zinc-850/80 border-zinc-800 text-white hover:bg-zinc-800 hover:text-blue-400' 
                        : 'bg-blue-50 border-blue-100 text-[#0052cc] hover:bg-blue-100'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-3 py-1 border rounded-lg text-xs font-semibold transition-all active:scale-[0.97] flex items-center gap-1 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                      darkMode 
                        ? 'bg-zinc-900 border-zinc-800 hover:bg-zinc-800 text-zinc-400 hover:text-white' 
                        : 'bg-zinc-100 border-zinc-200 hover:bg-zinc-200 text-zinc-650 hover:text-zinc-900 shadow-2xs'
                    }`}
                  >
                    리포트 교체
                  </button>
              ...
              {step === 2 && (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all active:scale-[0.97] flex items-center gap-1.5 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                    darkMode 
                      ? 'border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-900/80' 
                      : 'border-zinc-200 text-zinc-650 hover:text-zinc-900 hover:bg-zinc-50 shadow-2xs'
                  }`}
                >
                  <ArrowLeft className="h-3 w-3" />
                  역할 다시 선택
          ...
          <div className={`flex-1 overflow-y-auto p-6 space-y-4 scrollbar-thin ${
              ...
              return (
                ...
                >
                  ...
                  <div className={`p-4 text-xs leading-relaxed rounded-2xl shadow-md ${
                    isModel 
                      ? (darkMode ? 'bg-zinc-950/80 border border-zinc-850 text-zinc-200' : 'bg-zinc-100 border border-zinc-200/80 text-zinc-800') 
                      : (darkMode ? 'bg-[#0052cc] text-white' : 'bg-[#2b52b4] text-white')
                  } ${isModel ? 'rounded-tl-none' : 'rounded-tr-none'}`}>
                    {isModel ? (
                      <div className={`prose ${darkMode ? 'prose-invert text-zinc-200' : 'text-zinc-800'} max-w-none leading-relaxed font-sans`}>
            ...
            {step === 1 && (
              <div className="flex flex-col sm:flex-row gap-4 w-full max-w-2xl mt-4 motion-safe:animate-fade-in self-start">
                ...
                <button
                  ...
                  className={`flex-1 text-left p-5 rounded-2xl transition-all active:scale-[0.98] flex flex-col justify-between group cursor-pointer shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                    darkMode 
                      ? 'bg-zinc-950/60 hover:bg-zinc-950/90 border border-zinc-850 hover:border-zinc-700/80' 
                      : 'bg-white hover:bg-zinc-50/50 border border-zinc-200 hover:border-blue-300'
                  }`}
                >
                  <div className="flex flex-col gap-3">
                    <div className={`h-9 w-9 rounded-xl flex items-center justify-center border group-hover:scale-105 transition-all ${
                      darkMode 
                    ...
                    <div>
                      <h3 className={`font-extrabold text-sm ${darkMode ? 'text-white' : 'text-zinc-900'} group-hover:text-blue-500 transition-colors`}>Candidate Mode</h3>
                      <p className={`text-[11px] ${darkMode ? 'text-zinc-400' : 'text-zinc-600'} font-medium mt-1 leading-relaxed`}>
                        VOTEST 응시 후 개인별 세일즈 지표와 컴플라이언스 항목을 검토하고, 맞춤형 롤플레잉 훈련을 진행합니다.
                      </p>
                    </div>
                  </div>
                  <div className={`mt-4 py-2 w-full rounded-lg text-xs font-bold text-center transition-all flex items-center justify-center gap-1.5 shadow-sm ${
                    darkMode 
                      ? 'bg-zinc-900 border border-zinc-800 text-zinc-300 group-hover:bg-[#0052cc] group-hover:text-white group-hover:border-[#0052cc]' 
                      : 'bg-zinc-100 border border-zinc-200 text-zinc-650 group-hover:bg-[#2b52b4] group-hover:text-white group-hover:border-[#2b52b4]'
                  }`}>
                    <span>선택하고 계속하기</span>
                ...
                >
                  <div className="flex flex-col gap-3">
                    ...
                    <div>
                      <h3 className={`font-extrabold text-sm ${darkMode ? 'text-white' : 'text-zinc-900'} group-hover:text-blue-500 transition-colors`}>Manager Mode</h3>
                      <p className={`text-[11px] ${darkMode ? 'text-zinc-400' : 'text-zinc-600'} font-medium mt-1 leading-relaxed`}>
            ...
            {step === 2 && (
              <div className={`p-5 rounded-2xl border flex flex-col gap-4 max-w-md mt-2 self-start motion-safe:animate-fade-in ${
                darkMode ? 'bg-zinc-950/80 border-zinc-850' : 'bg-white border-zinc-200 shadow-lg'
              }`}>
                <div 
                  ...
                  className={`border border-dashed rounded-xl p-6 text-center transition-all ${
                    dragActive 
                      ...
                      : (darkMode ? 'border-zinc-800 hover:border-zinc-750 bg-zinc-900/10' : 'border-zinc-200 hover:border-blue-300 bg-zinc-50/50')
                  }`}
                >
                  <Upload className={`h-5 w-5 mx-auto mb-2 ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`} />
                  <p className={`text-xs font-bold ${darkMode ? 'text-white' : 'text-zinc-800'}`}>VOTEST 평가서 PDF 드래그 앤 드롭</p>
                  <p className={`text-[10px] ${darkMode ? 'text-zinc-400' : 'text-zinc-500'} mt-1`}>또는 컴퓨터에서 파일 선택</p>
                  ...
                  <button
                    ...
                    className={`mt-3 px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-all active:scale-[0.97] cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                      darkMode 
                        ? 'border-zinc-800 bg-zinc-900 text-zinc-300 hover:bg-zinc-850 hover:text-white' 
                        : 'border-zinc-200 bg-white text-zinc-650 hover:bg-zinc-100 hover:text-zinc-850'
                    }`}
                  >
                    파일 선택하기
                  </button>
                  ...
                  {selectedFileName && (
                    <div className="mt-3 text-[11px] text-emerald-500 font-bold flex items-center justify-center gap-1">
                ...
                <div className="flex flex-col gap-1.5">
                  <textarea
                    ...
                    className={`w-full px-3 py-2 rounded-lg text-[11px] border outline-none focus:border-blue-500/40 focus-visible:ring-2 focus-visible:ring-blue-500/40 transition-all resize-none font-sans ${
                      darkMode 
                        ? 'border-zinc-850 bg-zinc-900 text-zinc-200 placeholder-zinc-500' 
                        : 'border-zinc-200 bg-zinc-50 text-zinc-800 placeholder-zinc-400 focus:bg-white'
                    }`}
                  />
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 px-3 py-2 rounded-lg text-xs font-bold bg-[#0052cc] hover:bg-[#004bb3] text-white disabled:opacity-40 disabled:pointer-events-none transition-all active:scale-[0.97] flex items-center justify-center gap-1 shadow-lg shadow-blue-500/10 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950"
                  >
                    {isAnalyzing ? (
                      <>
                  ...
                  <button
                    ...
                    disabled={isAnalyzing}
                    className={`px-3 py-2 rounded-lg text-xs font-bold transition-all active:scale-[0.97] cursor-pointer flex items-center justify-center gap-1 border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                      darkMode 
            ...
            {chatLoading && (
              <div className="self-start flex flex-col max-w-[80%] w-full animate-pulse">
                ...
                <div className={`border p-4 rounded-2xl rounded-tl-none flex flex-col gap-2 ${
                  darkMode ? 'bg-zinc-950/85 border-zinc-850' : 'bg-zinc-100 border-zinc-200'
                }`}>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-[10px] font-bold ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>피드백 구상 중...</span>
                  </div>
                  <div className={`h-1 rounded-md w-3/4 ${darkMode ? 'bg-zinc-850' : 'bg-zinc-200'}`} />
                  <div className={`h-1 rounded-md w-1/2 ${darkMode ? 'bg-zinc-850' : 'bg-zinc-200'}`} />
                </div>
              </div>
            )}
              ...
              <div className="flex items-center gap-2 overflow-x-auto py-1 scrollbar-none">
                {customChips[currentView].map((qa, index) => (
                  <button
                    ...
                    className={`shrink-0 text-[11px] px-3.5 py-1.5 border hover:border-blue-500 transition-all active:scale-[0.97] flex items-center gap-1.5 cursor-pointer disabled:opacity-40 disabled:pointer-events-none rounded-full font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                      darkMode 
                        ? 'bg-zinc-900 border-zinc-800 text-zinc-300 hover:bg-[#0052cc]/10 hover:text-white' 
                        : 'bg-white border-zinc-200 text-zinc-700 hover:bg-blue-50 hover:text-blue-650 shadow-xs'
                    }`}
                  >
                    <span>{qa.label}</span>
                    <span className={`text-[9px] font-bold ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>&rarr;</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {step === 3 ? (
            <div className={`border-t flex items-center shrink-0 focus-within:ring-2 focus-within:ring-blue-500/40 focus-within:ring-inset transition-shadow ${
              ...
              <input
                ...
                disabled={chatLoading}
                className={`flex-1 px-6 py-4 bg-transparent placeholder-zinc-400 text-xs border-none outline-none focus:ring-0 focus:outline-none disabled:cursor-not-allowed font-sans ${
              ...
              <button
                ...
                className={`h-[52px] w-[52px] border-l bg-transparent hover:bg-blue-500/10 text-zinc-300 hover:text-white disabled:opacity-30 disabled:pointer-events-none transition-all active:scale-[0.97] flex items-center justify-center font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950 ${
                  darkMode ? 'border-zinc-850/60' : 'border-zinc-200'
                }`}
              >
            ...
            <div 
              className={`border-t p-4 flex items-center justify-center select-none cursor-pointer transition-all gap-2 text-xs font-semibold ${
                darkMode ? 'border-zinc-800/60 bg-zinc-950 text-zinc-400 hover:bg-zinc-900/20' : 'border-zinc-200 bg-zinc-50 text-zinc-500 hover:bg-zinc-100/60'
              }`}
              onClick={() => document.getElementById('file-upload-input-inline')?.click()}
            >
            ...
            <div className={`border-t p-4 flex items-center justify-center text-xs font-semibold ${
              darkMode ? 'border-zinc-800/60 bg-zinc-950 text-zinc-400' : 'border-zinc-200 bg-zinc-50 text-zinc-500'
            }`}>
      ...
      {/* Aesthetic Footer */}
      <footer className={`w-full max-w-7xl mx-auto px-6 h-12 flex items-center justify-center border-t text-[10px] font-semibold uppercase tracking-widest shrink-0 ${
        darkMode ? 'border-zinc-900/40 text-zinc-400' : 'border-zinc-200 text-zinc-500'
      }`}>
